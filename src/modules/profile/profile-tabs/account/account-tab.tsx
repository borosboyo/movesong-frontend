  ...
  return (
    <TabsContent value="account">
      <Card>
        <CardHeader>
          {subscription ? (
            <PremiumHeader subscription={subscription} setSubscription={setSubscription} />
          ) : (
            <FreeHeader />
          )}
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          {connections.length === 0 ? (
            <div className={`flex justify-center items-center`}>
              <span className={`text-xl text-muted-foreground font-bold`}>{t('profile.accountTab.noConnections')}</span>
            </div>
          ) : (
