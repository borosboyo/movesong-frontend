...
export function HistoryTab() {
  ...
  useEffect(() => {
    setLoading(true);
    if (user?.email) {
      profileService.getTransformsByMovesongEmail(user.email)
        .then((resp) => {
          if (resp.transforms) {
      ...
      <CardHeader>
        <div className={`grid w-full grid-cols-2`}>
          ...
          <div className={`flex w-auto justify-end`}>
            <Button className={`primaryButton`} onClick={handleNewTransformClick}>{t('profile.historyTab.newTransformButtonText')}</Button>
          </div>
        </div>
      </CardHeader>
          ...
          <div className={`flex flex-col gap-2`}>
            {loading
              ...
              transforms.map((transform) => (
                <Transform key={transform.id} transform={transform} />
              ))}
            {transforms.length === 0 &&
