...
export function LandingTitle() {
  ...
  return (
    <>
      <div className={`darkBg md:w-full lg:w-full flex flex-col justify-center items-center pt-16 lg:pt-32 text-center`}>
        <h1
          className={`scroll-m-20 w-3/4 lg:w-full text-4xl font-extrabold tracking-tight lg:text-7xl ${useTitleTheme()}`}>{t('landing.landingTitle.header')}
        </h1>
        <LogoRibbon />
        <span className={`text-xl lg:text-3xl font-bold max-w-3xl text-white mx-2`}>{t('landing.landingTitle.subtitle')}</span>
...
function LogoRibbon() {
  return (
    <div className={`flex flex-row mt-5 mb-5 gap-5`}>
      <img className={`w-26 h-10 object-cover floatLogo floatLogo-1`} src={youtubeMusicTextIcon} alt={`youtube-music`} />
