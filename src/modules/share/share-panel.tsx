...
export function SharePanel() {
  ...
  useEffect(() => {
    ...
    if (shareId) {
      shareService
        .getShareById(parseInt(shareId))
        .then((resp) => {
          if (resp?.share?.playlistId && resp?.share?.movesongEmail) {
            setShare(resp.share);
            if (!resp.share.visible && resp.share.movesongEmail !== user?.email) {
              navigate('/movesong-frontend/404');
            }
            if (resp.share?.sharePlatformType === 'YOUTUBE') {
              shareService
                .getItemsInYoutubePlaylist(resp.share.playlistId, resp.share.movesongEmail)
                .then((resp) => {
                  if (resp.items) {
              ...
              shareService
                .getItemsInSpotifyPlaylist(resp.share.playlistId, resp.share.movesongEmail)
                .then((resp) => {
                  if (resp.items) {
                    setPlaylistItems(resp.items);
                  }
  ...
  const handleImportClick = () => {
    if (share?.sharePlatformType === 'YOUTUBE') {
      transformService
        .getUserYoutubePlaylistByPlaylistId(share!.movesongEmail!, share!.playlistId!)
        .then((resp) => {
      ...
      transformService
        .getUserSpotifyPlaylistByPlaylistId(share!.movesongEmail!, share!.playlistId!)
        .then((resp) => {
          setSelectedPlaylist(resp.playlist);
          setSource(share!.sharePlatformType!);
          setSelectedPlaylistId(share!.playlistId!);
        ...
        <div className={`flex justify-center items-start w-full my-10`}>
          <Card className={`w-[400px] relative`}>
              ...
              <Dialog>
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>{t('share.editShareDialog.text')}</DialogDescription>
                  </DialogHeader>
                          ...
                          render={({ field }) => (
                            <FormItem className={`grid grid-cols-4 items-center gap-4`}>
                              <FormLabel className="text-right">
                                {t('share.editShareDialog.playlistNameLabel')}
                              </FormLabel>
                              <FormControl>
                      ...
                      <div className={`flex gap-2 col-span-3`}>
                        {avatars.map((avatar) => (
                          ...
                          >
                            <Avatar
                              key={avatar.id}
                              className={`${tempSelectedAvatar === avatar.background ? 'ring-4 ring-primary' : ''}`}
                            >
            ...
            <CardContent className="aspect-square justify-center p-6">
              <div className={`flex flex-col items-center gap-2`}>
                ...
                <span className={`w scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-3xl`}>
                  {share?.sharedPlaylistName}
                </span>
                <CardDescription>
                  {share?.movesongEmail} {t('share.sharedWithYou')}
                </CardDescription>
                <span className={`w scroll-m-20 text-s font-extrabold tracking-tight lg:text-s`}>
                  {t('share.openOn')}
                </span>
            ...
            <CardFooter className={`flex justify-center items-center`}>
              <Button className={`primaryButton`} onClick={handleImportClick}>
                {t('share.importIntoMyLibraryButtonText')}
              </Button>
            </CardFooter>
          </Card>
        </div>
