...
export function SourcePlaylistTab() {
  const { t } = useTranslation();
  const { user } = useAuth();
  const handleError = useHandleError();
  const [playlists, setPlaylists] = useState<PlaylistDto[]>([]);
  const [loading, setLoading] = useState(false);
  const { source } = useSync();
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      if (source === 'YOUTUBE') {
        transformService
          .getUserYoutubePlaylists(user?.email)
          .then((resp) => {
            if (resp.playlists) {
        ...
        transformService
          .getUserSpotifyPlaylists(user?.email)
          .then((resp) => {
            if (resp.playlists) {
              setPlaylists(resp.playlists);
            }
      ...
      <CardContent className="aspect-square justify-center p-6">
        <div className={`flex flex-col items-center mb-6`}>
          <span className={`scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-3xl`}>
            {t('transform.playlistTab.header')}
          </span>
          ...
          <div className={`flex flex-col gap-2`}>
            ...
            {playlists.map((playlist) => (
              <div key={playlist.id} className={`flex flex-row items-center gap-2`}>
                <SourceCollapsiblePlaylist platform={source} playlist={playlist} />
              </div>
            ))}
          </div>
        </ScrollArea>
