...
export function SourceCollapsiblePlaylist({ playlist, platform, }: { playlist: PlaylistDto | undefined, platform: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [playlistItems, setPlaylistItems] = useState<PlaylistItemDto[]>([]);
  const [loading, setLoading] = useState(false);
  const { user } = useAuth();
  const { sourcePlaylistId, setSourcePlaylistId, setSelectedSourcePlaylist } = useSync();
  ...
  return (
    ...
    >
      <div className={`flex flex-row justify-content items-center gap-2`}>
        <Checkbox
          checked={sourcePlaylistId === playlist?.id}
