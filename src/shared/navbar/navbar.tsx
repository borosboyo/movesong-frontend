  ...
  return <>
    <header className={`bg-[size:20px_20px] top-* flex h-20 w-full shrink-0 items-center px-4 md:px-6 darkBg`}>
      <Sheet>
        ...
        <SheetContent side="left" className={`bg-white`}>
          <div className={`grid grid-cols-1 gap-2 py-6`}>
            <div className={`mr-6`}>
              <h1 className={`scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-4xl select-none`}>{t('navbar.title')}</h1>
            </div>
      ...
      <div className={`mr-6 hidden lg:flex`}>
        <h1 className={`pt-5 scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-4xl ${useTitleTheme()} select-none`}>{t('navbar.title')}</h1>
      </div>
